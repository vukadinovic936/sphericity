...
#  res.cox <- coxph(Surv(years_without_incident, status) ~ hw + age + sex + BSA + pulse_rate + hypertension, data = hw_20_80)
#  hw_df <- with(hw_20_80,
...
#  ggsave( paste(substr(csv_name,1,nchar(csv_name)-3),sep='',"png"))
  ...
  myocardial = read.csv(csv_name)
  myocardial['hw'] = 1/myocardial['hw']
  myocardial['norm_hw'] = scale(myocardial['hw'])
  res.cox <- coxph(Surv(days_without_incident, status) ~ norm_hw + age + sex + BSA + pulse_rate + hypertension, data = myocardial)
